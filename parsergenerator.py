...
class ParserGenerator(object):
    ...
    def generate_file(self, parsername=None, outfilepath=None):
        self.__logger.info(u"Start generating Parser. \"{0}\"".format(self.pegfilepath))

        self.parser.parse_file(self.pegfilepath, self.fileEncoding)

        tree = self.parser.get_tree()

        self.__logger.debug(u"Check tree start.")
        ...
        preparserstr = u"class " + parsername + u"(Parser):\n\n" \
                                                u"    def __init__(self):\n" \
                                                u"        Parser.__init__(self)\n" \
                                                u"        self.top = self.p_{0}\n" \
                                                u"        self.toptypename = \"{1}\"\n" \
                                                u"".format(self.rootname.lower(), self.rootname)
        # 構文辞書の追加
        dict_str = (",\n" + u" " * 25).join(["\"" + d_name + "\": self.p_" + d_name.lower()
                                             for d_name in self.__definition])
        str_def_list = u"        self.def_dict = {" + dict_str + "}\n"

        # サブ構文返還バックアップ辞書の追加
        str_subdef_list = ""
        if len(self.__sub_definition) > 0:
            subdict_str = (",\n" + u" " * 28).join(["\"p_" + sd_name.lower() + "\": self.p_" + sd_name.lower()
                                                    for sd_name in self.__sub_definition])
            str_subdef_list = u"        self.def_bk_dict = {" + subdict_str + "}\n"
            subtype_str = (u" " * 32).join(["\"" + sd_name + "\",\n" for sd_name in self.__sub_definition])
            if len(self.__sub_definition) == 1:
                subtype_str = u"\"" + self.__sub_definition[0] + "\""
            str_subdef_list += u"        self.def_subtypename = [" + subtype_str + "]\n"

        self.__logger.debug(u"Output to file \"{0}\" start.".format(outfilepath))
        with open(outfilepath, "w", encoding='utf-8') as fout:
            fout.write(impstr + preparserstr + str_def_list + str_subdef_list + strparser)

        self.__logger.info(u"Generating Parser ended successfully.")
        return impstr + preparserstr + strparser

    def travel_generate_file(self, tree, level=0):
        u"""Node 探索し generator を作成する
    ...
    def get_literal_value(self, tree):
        ...
        if tree.type == "SingleQuotesLiteral":
            ...
            return "u'" + cont[0].get_str(d).replace(u"\\", u"\\\\").replace(u"'", u"\\'") + "'"
    ...
    def get_reg_value(self, tree):
        ...
        if tree.type == "SingleQuotesLiteral":
            ...
            if "m" not in opt_str:
                opt_str += "M"
            reg_opt_dic = {'M': 're.M', 'X': 're.X', 'A': 're.A', 'I': 're.I', 'S': 're.S'}
    ...
    def find_left_recursive_loop_list(self):
        ...
        while check_def_name not in loop_list:
            ...
            if not isinstance(next_def_name, str):
                err_msg = u"!Unexpected Error! @search_unresolve_call {0}: {1} -> {2}" \
